...
type ThemeHelpersModule = {
  getResolvedThemeColors(themeName?: string): Record<string, string>;
  getThemeExportColors(themeName?: string): PiThemeState["exportColors"];
  isLightTheme(themeName?: string): boolean;
...
async function getThemeHelpers() {
  if (!themeHelpersPromise) {
    ...
    themeHelpersPromise = import(
      pathToFileURL(themeModulePath).href
    ) as Promise<ThemeHelpersModule>;
  }
...
function hexToRgb(hex: string | undefined) {
  const match = hex ? /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(hex.trim()) : null;
...
function getThemeEntries(
  ...
  return [
    ...
    (theme, index, themes) => themes.findIndex((item) => item.name === theme.name) === index,
  );
}
...
export async function loadPiThemeStateInHost(projectPath?: string | null): Promise<PiThemeState> {
