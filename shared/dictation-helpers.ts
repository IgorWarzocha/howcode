...
const englishWhisperModelPattern = /(^|[._-])en($|[._-])/
const languageSubtagSeparatorPattern = /[-_]/
const whisperEncoderFilePattern = /^(.*)-encoder(\.int8)?\.onnx$/
...
export function inferWhisperLanguage(modelId: string) {
  return englishWhisperModelPattern.test(modelId) ? 'en' : null
}
...
export function normalizeWhisperLanguage(language: string | null | undefined) {
  ...
  const primarySubtag = normalized.split(languageSubtagSeparatorPattern)[0]?.trim()
...
export function resolveWhisperModelFilesFromFilePaths(
  ...
  const encoderCandidates = filePaths
    .map((filePath) => {
      const match = path.basename(filePath).match(whisperEncoderFilePattern)
      if (!match) {
        return null
      }
